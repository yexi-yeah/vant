...
test('should minHeight 100 and maxHeight 0.6 innerHeight when anchors props do not', async () => {
  ...
  expect(wrapper.element.style.transform).toContain('-100px');
  wrapper.destroy();
});
...
test('should drag adsorption effect when anchors props is [100, 200, 400]', async () => {
  ...
  await later();
  expect(wrapper.element.style.transform).toContain('-200px');

  // drag -300
  await triggerDrag(wrapper.find('.van-floating-panel__header'), 0, -300);
  await later();
  expect(wrapper.element.style.transform).toContain('-400px');
